...
public class ArtHall {
	...
	public void showTheaterProgram(String theaterName)
	{
		for(int j=0;j<this.program.length;j++) {
			if (theaterName.equals(type[j].name)) {
				program[j].showURL();
				break;
			}
		}
	...
	public int updateProgram(String theaterName, ReservedWebProgram program) {
		...
		for(TheaterType tt : this.type) {
			if (tt.name.equals(theaterName)) {
				for(int i = 0; i < this.program.length; i++) {
					if (this.program[i].program.equals(program.program)) {
						this.program[i].type = tt;
						return i;
					}
				}
			}
	...
	public static void main(String[] args) {
        ...
        String chromdOption = " -window-size=1400,1040 --window-position=3800,100 --force-device-scale-factor=0.3 ";
        try
	        {
